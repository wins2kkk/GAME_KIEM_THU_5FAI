...
public class TransparentDetection : MonoBehaviour
{
    ...
    {
        yield return Fade(spriteRenderer, fadeTime, startValue, targetTransparency);
    }

    private IEnumerator FadeRoutine(Tilemap tilemap, float fadeTime, float startValue, float targetTransparency)
    {
        yield return Fade(tilemap, fadeTime, startValue, targetTransparency);
    }

    private IEnumerator Fade(object renderer, float fadeTime, float startValue, float targetTransparency)
    {
        float elapsedTime = 0;
        while (elapsedTime < fadeTime)
        {
            ...
            else if (renderer is Tilemap tm)
            {
